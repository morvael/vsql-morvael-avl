/*
 * $Id: TerrainMap.java 3854 2008-07-25 11:14:23Z morvael $
 *
...
public class TerrainMap {
	...
	public void setEdgeTerrainType(ArrayList<EdgeRef> edges,
		...
		for (EdgeRef edgeRef : edges) {
			setEdgeTerrainType(new TerrainEdge(edgeRef, terrain));
		}
	}
	...
	public void setLineTerrainType(ArrayList<LineRef> lines,
		...
		for (LineRef lineRef : lines) {
			setLineTerrainType(new TerrainLine(lineRef, terrain));
		}
	}
	...
	public TerrainLine getLineTerrain(HexRef hexPos1, HexRef hexPos2) {
		...
		return (TerrainLine) lineMap.get(key);
	}
	...
	public TerrainAttribute getAttributeTerrain(HexRef hexPos) {
		return (TerrainAttribute) attributeMap.get(hexPos);
	}
	...
	public GeneralPath getEdgePoly(String terrainType) {
		...
		GeneralPath poly = (GeneralPath) edgePoly.get(terrainType);
		if (poly == null) {
			rebuildEdgeAreas();
			poly = (GeneralPath) edgePoly.get(terrainType);
		}
		return poly;
	}
	...
	public GeneralPath getLinePoly(String terrainType) {
		...
		GeneralPath poly = (GeneralPath) linePoly.get(terrainType);
		if (poly == null) {
			rebuildLineAreas();
			poly = (GeneralPath) linePoly.get(terrainType);
		}
		return poly;
	}
	...
	public void save() {
		...
		for (TerrainEdge edge : edgeMap.values()) {
			...
			if (ref.getColumn() < ref.getColumn2()
					|| (ref.getColumn() == ref.getColumn2() && ref.getRow() <= ref
							.getRow2())) {
		...
		final ArchiveWriter writer = GameModule.getGameModule()
				.getArchiveWriter();
